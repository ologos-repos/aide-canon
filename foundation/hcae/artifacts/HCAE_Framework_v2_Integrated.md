...
Enterprise AI projects fail at rates between 70% and 95%, yet the dominant response remains horizontal-axis interventions: more data, larger models, better retrieval. This paper argues that these failures are primarily grounding-axis problems misdiagnosed as infrastructure problems. AI systems lack access to the purposes they serve and the wholes their outputs enter. No amount of scaling addresses this structural deficit.

The Human-Curated, AI-Enabled (HCAE) framework provides a design discipline for reliable AI deployment. It specifies four tiers of deployment architecture, each providing a different mechanism for supplying the grounding that AI systems structurally lack: User-Curated (UCAE) for low-stakes ideation, Professional-Curated (PCAE) for routine domain work, Expert-Curated (ECAE) for high-stakes analysis, and Synthesis-Curated (SCAE) for domains where formal verification is tractable. The framework is a decision tool, not a maturity model. Higher tiers are not universally better; they are appropriate for different task profiles. The goal is matching deployment architecture to task requirements, not maximizing tier.

Building on the AI Dunning-Kruger (AIDK) framework, which establishes the structural epistemic limitations of large language models, this paper operationalizes those findings into deployment guidance. It addresses hybrid deployments and tier transitions, strengthening mechanisms for ECAE in non-formalizable domains, and provides worked examples of SCAE in mathematical proof and safety-critical systems. The three-axis model (horizontal, vertical, grounding) explains why scaling and reasoning improvements alone cannot resolve the reliability problem. Grounding must be supplied externally because it cannot be generated internally.
...
HCAE is not a maturity model. Higher tiers are not better in the abstract. They are appropriate for different task profiles.

UCAE is the right answer for brainstorming, early-stage ideation, and low-stakes drafting. Deploying ECAE for tasks that need only stimulation wastes expert capacity and slows iteration. The goal is not to maximize tier but to match tier to task.
...
Enterprise AI fails at rates between 70% and 95%. The standard response has been horizontal-axis interventions: more data, larger models, better retrieval. This response is inadequate because it addresses the wrong problem.

AI systems suffer from a structural grounding deficit. They lack access to the purposes they serve (teleological blindness) and the wholes their outputs enter (mereological blindness). No amount of scaling, no sophistication of architecture, no refinement of training resolves this deficit. Grounding must be supplied externally because it cannot be generated internally.

The Human-Curated, AI-Enabled (HCAE) framework provides a design discipline for addressing this structural limitation. It specifies four tiers of deployment architecture, each providing a different mechanism for supplying grounding:

- **UCAE** for low-stakes ideation where grounding is unnecessary
- **PCAE** for routine professional work with genuine validation
- **ECAE** for high-stakes analysis requiring expert judgment
- **SCAE** for domains where formal verification is tractable

The framework is a decision tool, not a maturity model. Higher tiers are not universally better; they are appropriate for different task profiles. The goal is matching deployment architecture to task requirements. Selection, not aspiration.

The three-axis model explains why this framework is necessary and why alternatives fail. Horizontal solutions (more infrastructure) cannot address grounding problems. Vertical solutions (better reasoning) improve system quality without supplying grounding. Only grounding-axis interventions, which HCAE represents, address the structural deficit.

The implications are practical and immediate:

**For organizations:** Use the tier-matching criteria as a pre-deployment assessment. Invest in building tier-appropriate infrastructure before scaling deployment. Accept that reliable AI requires human grounding and budget accordingly.

**For practitioners:** Recognize that AI amplifies human capacity without replacing human judgment. Maintain appropriate skepticism. Design workflows that preserve human epistemic authority rather than eroding it.

**For researchers:** The HCAE framework invites empirical investigation. Measure IDKE magnitude across tiers. Compare failure modes in matched versus mismatched deployments. Quantify the value of strengthened ECAE mechanisms.

**For regulators:** Tiered oversight maps naturally to risk-based regulation. UCAE disclosure requirements, PCAE professional standards, ECAE expert certification, SCAE mandatory verification for high-risk applications.

The path forward is not more scaling. The path forward is design discipline that acknowledges what AI systems are and are not. They are powerful tools for generation and search. They are not epistemic agents with access to truth. They require grounding that only humans, operating at appropriate levels of expertise, can provide.

The framework offered here is a start. It will require refinement as deployment experience accumulates, as technology evolves, and as regulatory frameworks mature. But the core insight will persist: grounding is the constraint, and human judgment is the solution.

The choice is not between AI and no AI. The choice is between AI deployed with appropriate grounding and AI deployed without it. The former produces value. The latter produces the 70-95% failure rates that currently characterize enterprise AI.

We can do better. HCAE shows how.
