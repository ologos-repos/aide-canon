...
| **OrdSA** | Ordinal Systems Architecture — JD's published authority-layering framework. *"Authority flows downward, evidence flows upward."* Public at [osa-ai-org/ordsa-ai](https://github.com/osa-ai-org/ordsa-ai) |
...
| R9 | **Operational dependency on Ollama for galley semantic search** — Hermetic galley uses Ollama embeddings; if Ollama is unreachable, falls back to keyword search but loses semantic-tool-discovery quality. | Low | Already mitigated in galley pattern (graceful degradation); for AEON deployment, run an Ollama replica per host or fall through to a hosted embedding provider when local Ollama unavailable. |
