...
- **AIDE ahead** — **only on two specific constructs**: (1) **OrdSA ordinal authority** (O0–O6 authority-down / evidence-up altitude layering — IBM has policy + RBAC + lifecycle gates, but not authority-*altitude*), and (2) the **OAgents per-action behavioral envelope** over a *running* agent ([OAgents §10](../../aide-vocabulary-map.md) names this as the layer that sits "above any framework"; IBM governs the model/agent *object* and its *lifecycle*, not each in-flight action against a deontic envelope). Also ahead on HCAE operator-as-curator and on vendor-neutral conformance criteria.
