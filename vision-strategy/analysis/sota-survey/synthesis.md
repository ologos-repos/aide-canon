...
- **The interface wires are settled — consume them, don't reinvent.** MCP (tool), A2A (agent interop), OTel-GenAI (evidence), OAuth/RFC-8693/SPIFFE (delegation). The canon already **consumes** MCP (α1 skills as MCP servers) and **adopted+extended** OTel-GenAI as its evidence schema ([ADR-EA-0027](../../../decisions/ADR-EA-0027-introduce-workflow-orchestration-pattern.md)). The differentiator is the authority/envelope layer *above* the wire.
- **Orchestration converges on [ADR-EA-0027](../../../decisions/ADR-EA-0027-introduce-workflow-orchestration-pattern.md)** — LangGraph / LlamaIndex / Mastra Workflows are the same event-driven shape; the canon's distinctive is the **envelope-refinement lattice `envelope(child) ⊑ envelope(parent)`** — which the [workflow-orchestration pattern](../../../patterns/workflow-orchestration.md) records as **unbuilt across all known implementations** (the symmetry finding: Claude Code = convention, FOrCE = per-action-type, nobody enforces ⊑). First to build the enforced subset-test owns the first realized lattice.
