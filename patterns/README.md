...
| [ai-aide-capability-model.md](ai-aide-capability-model.md) | Proposed (pending [ADR-EA-0028](../decisions/ADR-EA-0028-introduce-ai-aide-capability-model-pattern.md)) | Standards-aligned, gap-filling capability catalog for state-of-the-art AI-aide evaluation; maps capabilities to external standards where they cover the behavior, defines AIDE-native constructs where standards are insufficient, cites exemplars such as Hermetic as evidence without adopting implementation-local naming, and requires evaluation probes plus current-state honesty. |
