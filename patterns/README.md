...
| [governed-context-management.md](governed-context-management.md) | Proposed (see [ADR-EA-0019](../decisions/ADR-EA-0019-introduce-governed-context-management-pattern.md)) | Seven-section harness-owned context-management discipline — governance-pin invariant (4M/operating-mode/authority re-asserted every window, never compactable), per-model context budgeting, deterministic compaction, audited compaction events (`context.compacted` with `what_was_dropped`), Evidence re-hydration, inchstone decomposition as context hygiene, integrity-degraded autonomy fail-safe — distributed across MxM Morals/Memory + Inference/Runtime/Evidence planes |
